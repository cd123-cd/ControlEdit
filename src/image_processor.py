...
class VaeImageProcessor(ConfigMixin):
    ...
    def preprocess(
        self,
        image_sketch: PipelineImageInput,
        image_mask: PipelineImageInput,
        ...
        if isinstance(image[0], PIL.Image.Image):
            ...
            image_mask = self.pil_to_numpy(image_mask)  # to np
            image_mask = self.numpy_to_pt(image_mask)  # to pt
            mask=self.resize(mask, height, width, resize_mode=resize_mode)
            mask = self.pil_to_numpy(mask)//255  # to np
            mask = self.numpy_to_pt(mask)  # to pt
        ...
        if do_normalize:
            ...
            image_mask = self.normalize(image_mask)

        if self.config.do_binarize:
            image = self.binarize(image)

        return image, image_mask, mask
